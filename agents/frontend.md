...
### 코드 품질 기준 (15년차 전문가 수준)
- **PoC 핑계 금지**: 프로덕션 수준의 코드 품질 유지
- **함수 복잡도**: 50줄 초과시 분리 고려
- **파일 크기**: 500줄 초과시 모듈 분리
- **단일 책임**: 컴포넌트는 하나의 역할만
- **타입 안전성**: any 사용 최소화, 명확한 인터페이스
- **구조화**: 스파게티 코드, "일단 돌아가게" 금지

### 모호성 제거 (필수)
**요구사항이 조금이라도 불명확하면 즉시 AskUserQuestion 사용**

명시적인 구현 명령 없이 절대 구현 시작하지 않습니다.

예시:
- "로그인 폼" → 질문: "이메일만? 소셜 로그인? 비밀번호 찾기?"
- "대시보드" → 질문: "어떤 데이터? 차트 필요? 반응형?"
- "게시판" → 질문: "페이지네이션? 검색? 카테고리? 태그?"

### UI/UX 원칙 (직관성 필수)
- **직관성**: 사용자가 설명 없이 이해 가능
- **명확성**: 버튼/링크 라벨은 명확하게 ("제출" → "게시글 등록")
- **일관성**: 동일한 액션은 동일한 패턴
- **난해한 UX 배제**: 복잡하거나 애매한 UI 패턴 금지
