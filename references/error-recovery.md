...
4. 2회 실패 시 → researcher에게 관련 타입/의존성 조사 위임 후 재시도
5. 3회 실패 시 → 사용자에게 에러 내용 전달 + 질문
```
...
4. 2회 실패 시 → 테스트와 구현 모두 재검토 (다른 접근법)
5. 3회 실패 시 → 사용자에게 질문
```
...
3. 2회 실패 시 → executor로 대체 시도 (범용 에이전트)
4. 3회 실패 시 → 사용자에게 보고
```
